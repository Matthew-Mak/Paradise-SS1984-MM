...
/turf/simulated/hotspot_expose(exposed_temperature, exposed_volume, soh)
	...
	if(active_hotspot)
		if(soh)
			if(air_contents.gases.get(GAS_PLASMA) > 0.5 && air_contents.gases.get(GAS_OXYGEN) > 0.5)
	...
	if((exposed_temperature > PLASMA_MINIMUM_BURN_TEMPERATURE) && air_contents.gases.get(GAS_PLASMA) > 0.5)
		igniting = 1

	if(igniting)
		if(air_contents.gases.get(GAS_OXYGEN) < 0.5 || air_contents.gases.get(GAS_PLASMA) < 0.5)
			return 0
...
/obj/effect/hotspot/process()
	...
	if(!(location.air) || location.air.gases.get(GAS_PLASMA) < 0.5 || location.air.gases.get(GAS_OXYGEN) < 0.5)
		qdel(src)
		return
...
/proc/fireflash_s(atom/center, radius, temp, falloff)
	...
	while(open.len)
		...
		for(var/mob/living/L in T)
			L.adjust_fire_stacks(3)
			L.IgniteMob()
			L.adjust_bodytemperature((2 * (temp - L.bodytemperature)) / 3)
