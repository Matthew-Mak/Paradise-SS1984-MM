...
/mob/living/carbon/alien/check_breath(datum/gas_mixture/breath)
	...
	var/Toxins_pp = (breath.gases.get(GAS_PLASMA) / breath.total_moles()) * breath_pressure

	if(Toxins_pp > tox_detect_threshold) // Detect toxins in air
		adjust_alien_plasma(breath.gases.get(GAS_PLASMA)*250)
		throw_alert("alien_tox", /atom/movable/screen/alert/alien_tox)

		toxins_used = breath.gases.get(GAS_PLASMA)

	else
		clear_alert("alien_tox")

	//Breathe in toxins and out oxygen
	breath.gases.add(GAS_PLASMA, -toxins_used)
	breath.gases.get(GAS_OXYGEN, toxins_used)
