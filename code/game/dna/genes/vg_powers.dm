...
/obj/effect/proc_holder/spell/remotetalk/cast(list/targets, mob/living/carbon/human/user = usr)
	...
	if(user.mind?.miming) // Dont let mimes telepathically talk
		to_chat(user,"<span class='warning'>Вы не можете общаться, не нарушив свой обет молчания.</span>")
		return
	for(var/mob/living/target in targets)
		...
		if(target.dna?.GetSEState(GLOB.remotetalkblock))
			target.show_message("<span class='abductor'>Вы слышите голос [user.real_name]: [say]</span>")
		else
			target.show_message("<span class='abductor'>Вы слышите голос, который, кажется, эхом разносится по комнате: [say]</span>")
		user.show_message("<span class='abductor'>Вы проецируете свой разум на [(target in user.get_visible_mobs()) ? target.name : "неизвестную сущность"]: [say]</span>")
...
/obj/effect/proc_holder/spell/mindscan/Topic(href, href_list)
	...
	if(href_list["target"])
		...
		log_say("(TPATH to [key_name(target)]) [say]", user)
		if(target.dna?.GetSEState(GLOB.remotetalkblock))
			target.show_message("<span class='abductor'>Вы проецируете свой разум на [user.name]: [say]</span>")
		else
			target.show_message("<span class='abductor'>Вы заполняете пространство в своих мыслях: [say]</span>")
		user.show_message("<span class='abductor'>Вы слышите голос [target.name]: [say]</span>")
		for(var/mob/dead/observer/G in GLOB.player_list)
