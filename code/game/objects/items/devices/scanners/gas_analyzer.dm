...
/proc/atmos_scan(mob/user, atom/target, silent=FALSE, print=TRUE)
	...
	for(var/datum/gas_mixture/air as anything in airs)
		...
		if(total_moles > 0)
			...
			if(air.gases.get(GAS_OXYGEN))
				message += span_notice("Oxygen: [round(air.gases.get(GAS_OXYGEN), 0.01)] mol ([round(air.gases.get(GAS_OXYGEN) / total_moles*100, 0.01)] %)")
			if(air.gases.get(GAS_CDO))
				message += span_notice("Carbon Dioxide: [round(air.gases.get(GAS_CDO), 0.01)] mol ([round(air.gases.get(GAS_CDO) / total_moles*100, 0.01)] %)")
			if(air.gases.get(GAS_NITROGEN))
				message += span_notice("Nitrogen: [round(air.gases.get(GAS_NITROGEN), 0.01)] mol ([round(air.gases.get(GAS_NITROGEN) / total_moles*100, 0.01)] %)")
			if(air.gases.get(GAS_PLASMA))
				message += span_notice("Plasma: [round(air.gases.get(GAS_PLASMA), 0.01)] mol ([round(air.gases.get(GAS_PLASMA) / total_moles*100, 0.01)] %)")
			if(air.gases.get(GAS_N2O))
				message += span_notice("Nitrous Oxide: [round(air.gases.get(GAS_N2O), 0.01)] mol ([round(air.gases.get(GAS_N2O) / total_moles*100, 0.01)] %)")
			if(air.gases.get(GAS_AGENT_B))
				message += span_notice("Agent B: [round(air.gases.get(GAS_AGENT_B), 0.01)] mol ([round(air.gases.get(GAS_AGENT_B) / total_moles*100, 0.01)] %)")
