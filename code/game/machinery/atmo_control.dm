		...
		if(output > (SENSOR_SCAN_PRESSURE|SENSOR_SCAN_TEMPERATURE))
			...
			if(total_moles > 0)
				if(output & SENSOR_COMPOSITION_OXYGEN)
					signal.data["oxygen"] = round(100 * air_sample.gases.get(GAS_OXYGEN) / total_moles, 0.1)
				if(output & SENSOR_COMPOSITION_TOXINS)
					signal.data["toxins"] = round(100 * air_sample.gases.get(GAS_PLASMA) / total_moles, 0.1)
				if(output & SENSOR_COMPOSITION_NITROGEN)
					signal.data["nitrogen"] = round(100 * air_sample.gases.get(GAS_NITROGEN) / total_moles, 0.1)
				if(output & SENSOR_COMPOSITION_CO2)
					signal.data["carbon_dioxide"] = round(100 * air_sample.gases.get(GAS_CDO) / total_moles, 0.1)
				if(output & SENSOR_COMPOSITION_N2O)
					signal.data["nitrous_oxide"] = round(100 * air_sample.gases.get(GAS_N2O) / total_moles, 0.1)
			else
