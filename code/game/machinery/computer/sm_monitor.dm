...
/obj/machinery/computer/sm_monitor/ui_data(mob/user)
	...
	if(istype(active))
		...
		if(TM)
			data["SM_gas_O2"] = round(100*air.gases.get(GAS_OXYGEN)/TM, 0.01)
			data["SM_gas_CO2"] = round(100*air.gases.get(GAS_CDO)/TM, 0.01)
			data["SM_gas_N2"] = round(100*air.gases.get(GAS_NITROGEN)/TM, 0.01)
			data["SM_gas_PL"] = round(100*air.gases.get(GAS_PLASMA)/TM, 0.01)
			if(other_moles)
